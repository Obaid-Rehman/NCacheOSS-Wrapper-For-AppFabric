...
namespace Alachisoft.NCache.Data.Caching
{
   ...
   internal class DataFormatter
   {
       ...
       internal string[] SplitKeyAndRegion(string key)
       {
           ...
           returnList[1] = regionSplit[0].Remove(0,_prefix.Length);// region on index 1
           return returnList;
       }
       ...
       public LockHandle ConvertToNCacheLockHandle(DataCacheLockHandle dLockHandle)
       {
        ...
        internal CacheItem CreateCacheItem(object value, string region, TimeSpan timeOut)
        {
            MetaDataCapsule meta = MetaDataCapsule.Encapsulate(value, region);
            CacheItem _item = new CacheItem(meta);

            if (String.IsNullOrWhiteSpace(region))
            {
                meta.Group = returnDefaultRegionTag();
        ...
        internal CacheItem CreateCacheItem(object value, string region)
        {
            MetaDataCapsule meta = MetaDataCapsule.Encapsulate(value, region);
            CacheItem _item = new CacheItem(meta);
       ...
       {
            MetaDataCapsule meta = cItem.Value as MetaDataCapsule;
           ...
           _dataCacheItem.Version = ConvertToAPVersion(meta.CacheItemVersion);
           _dataCacheItem.Value = meta.Value;
       ...
       public DataCacheItemVersion ConvertToAPVersion(ItemVersion cItemVersion)
       {
       ...
       public ItemVersion ConvertToNCacheVersion(DataCacheItemVersion dItemVersion)
       {
           ItemVersion _nCVersion = dItemVersion._itemVersion;
           return _nCVersion;
       }
       #endregion
   }
